...
app.get('/fortunes/random', (req,res) => {
    res.json(fortunes[Math.floor(Math.random() * fortunes.lenght)]); 
});
...
app.delete('/fortunes/:id', (req,res) => {
    const {id}= req.params;
    const new_fortunes = fortunes.filter(f => f.id != id);   //give us those that do not match the id, if it matchtes the id the filter function will dedice not to place in the array 
    writeFortunes(new_fortunes);
    res.json(new_fortunes)  
})
module.exports = app;